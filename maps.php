...
$maps_query = "SELECT * FROM MAPS";

$maps_results = mysqli_query($connect, $maps_query);
...
if ($maps_results->num_rows > 0) {
        ...
        while($rowm = $maps_results->fetch_assoc()) 
        {
           
            $map_name = "{$rowm["MAP_NAME"]}";
            $map_type = "{$rowm["MAP_TYPE"]}";
            $map_img = "{$rowm["MAP_IMG"]}";
            $logo = "/owldb/images/";
            ...
            ++$count;
                ...
                echo "<div class=col-lg-4>";
                    ...
                    echo '<img class="card-img-top border border-dark" src="'.$logo.$map_img.'" alt="Logo" height="250">';
                        ...
                        echo '<h5 class="bg-success border border-dark card-title" style="text-align:center">'.$map_name.'</h5>';
                        echo "</div>";
                    echo "<ul class=list-group list-group-flush>";
                    echo '<li class="list-group-item bg-light"><h4><b>Type:</b> '.$map_type.'</h4></li>';
                    echo "</ul>";
                echo "</div>";
            echo "</div>";
