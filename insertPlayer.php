...
EOF1;
   ...
   /* if (!empty($PLAYER_ID)){ */
        $sql = "insert into players (PLAYER_ID, PLAYER_FIRSTNAME, PLAYER_LASTNAME, PLAYER_AGE, PLAYER_GENDER)" .
                    " values (DEFAULT, '$PLAYER_FIRSTNAME', '$PLAYER_LASTNAME', '$PLAYER_AGE', '$PLAYER_GENDER')";
        if ($connect->query ($sql) == TRUE) {
           
        }
        else
        {
            echo "Added Entry. Hit Back" . $connect->connect_error . "<br>";
            $action = 'PlayersAdm.php';
        }
    /* } 
    else
    {
        echo "Must provide a Player ID to enter a record <br>";
        $action = 'PlayersAdm.php';
    } */
